...
export function RestockListPage() {
  ...
  const load = useCallback(
    async (toPage: number) => {
      ...
      setLoading(true);
      try {
        const res = await listRestocks({ warehouseId, status, page: toPage, limit: LIMIT });
        setRows(res.rows);
        setPageInfo(res.pageInfo);
        setPage(toPage);
      } catch (err) {
        toaster.create({
          title: "Failed to load restocks",
        ...
        <Box borderWidth="1px" rounded="md" overflow="hidden">
          {loading ? (
            ...
            <Text p={6} color="gray.500">
              No {INBOUND_STATUS_LABELS[status].toLowerCase()} restocks.
            </Text>
          ) : (
              ...
              <Table.Body>
                {rows.map((row) => (
                  <Table.Row
                    key={row.id}
                    cursor="pointer"
                    onClick={() => navigate(`/inbound/restock/${row.id}`)}
                  >
